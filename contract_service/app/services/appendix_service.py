...
async def create_appendix(db: AsyncSession, contract_id: int, appendix_data: AppendixCreate):
...
async def get_appendices(db: AsyncSession, contract_id: int):
    result = await db.execute(select(Appendix).where(Appendix.contract_id == contract_id))
