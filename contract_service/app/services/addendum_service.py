...
async def create_addendum(db: AsyncSession, contract_id: int, addendum_data: AddendumCreate):
    addendum = Addendum(**addendum_data.dict(), contract_id=contract_id)  # ✅ Исправлено
    db.add(addendum)
    await db.commit()
...
async def get_addendums(db: AsyncSession, contract_id: int):
    result = await db.execute(select(Addendum).where(Addendum.contract_id == contract_id))
