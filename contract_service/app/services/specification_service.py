...
async def create_specification(
    db: AsyncSession, contract_id: int, spec_data: SpecificationCreate
):
...
async def get_specifications(db: AsyncSession, contract_id: int):
    result = await db.execute(
        select(Specification).where(Specification.contract_id == contract_id)
    )
...
async def get_contract_by_specification(db: Session, specification_id: int):
    ...
    if specification and specification.contract_id:
        return (
            ...
            .filter(models.Contract.id == specification.contract_id)
            .first()
        )
    return None
