...
from app.services.addendum_service import create_addendum, delete_addendum, get_addendums
...
async def create_new_addendum(contract_id: int, addendum: AddendumCreate, db: AsyncSession = Depends(get_db)):
    ...
    if settings.DEBUG:
        logger.debug("Создание нового дополнительного соглашения для контракта %d", contract_id)
...
async def list_addendums(contract_id: int, db: AsyncSession = Depends(get_db)):
    ...
    if settings.DEBUG:
        logger.debug("Получение списка дополнительных соглашений для контракта %d", contract_id)
...
async def remove_addendum(contract_id: int, addendum_id: int, db: AsyncSession = Depends(get_db)):
    ...
    if settings.DEBUG:
        logger.debug("Удаление дополнительного соглашения ID: %d из контракта %d", addendum_id, contract_id)
