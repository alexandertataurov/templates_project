const API_URL = "http://localhost:8000"; // 🔥 Проверь правильность URL
...
export const uploadTemplate = async (file: File, templateType: string, displayName: string): Promise<any> => {
  ...
  return apiLogger("POST", `${API_URL}/templates/upload`, formData);
};
...
export const updateTemplate = async (templateId: number, displayName: string, fields: string[]): Promise<any> => {
  ...
  return apiLogger("POST", `${API_URL}/templates/update`, formData);
};
...
export const deleteTemplate = async (templateType: string, displayName: string): Promise<any> => {
  ...
  return apiLogger("POST", `${API_URL}/templates/delete`, formData);
};


// 8️⃣ Получение списка шаблонов
export const getTemplates = async (): Promise<any[]> =>
  apiLogger("GET", `${API_URL}/templates/`);

// 🔹 Система мониторинга (Admin API)

export const getServerHealth = async () => apiLogger("GET", `${API_URL}/admin/health`);
export const getApiStats = async () => apiLogger("GET", `${API_URL}/admin/api/stats`);
export const getDbStatus = async () => apiLogger("GET", `${API_URL}/admin/db/status`);
export const getTaskStatus = async () => apiLogger("GET", `${API_URL}/admin/tasks/status`);
export const getServerConfig = async () => apiLogger("GET", `${API_URL}/admin/config`);
export const getLogs = async (level: string = "INFO") =>
  apiLogger("GET", `${API_URL}/admin/logs?level=${level}`);
